...
import { defineComponent, h, inject, markRaw, provide, reactive, ref, } from "vue";
const dialogs = reactive(new Set());
...
class DialogPromise extends Promise {
    constructor() {
        ...
        _DialogPromise_visible.set(this, ref(false));
        this.submit = (value) => {
    ...
    setup(props, { slots }) {
        ...
        return () => {
            ...
            return [
                ...
                ...Array.from(dialogs).map((dialog) => h(DialogItem, { dialog })),
            ];
        };
    },
});
...
export function useDialog() {
    const dialog = inject(dialogName);
    if (dialog) {
        return dialog;
    }
    else {
